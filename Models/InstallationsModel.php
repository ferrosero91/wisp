<?php
	class InstallationsModel extends Mysql{
      ...
      return $answer;
		...
		public function complete_installation(int $id,int $user,string $opening_date,string $closing_date,string $observation,int $state){
			$this->intId = $id;
			$this->intUser = $user;
      ...
      $this->strObservation = $observation;
			$this->strState = $state;
			$answer = "";
      $query = "INSERT INTO detail_facility(facilityid,technicalid,opening_date,closing_date,comment,state) VALUES(?,?,?,?,?,?)";
      ...
      if($insert){
		...
		public function view_installation(int $facility){
			...
			if(!empty($request_facility)){
				...
				$sql_tools = "SELECT t.id,t.facilityid,t.productid,t.quantity,t.price,t.total,t.product_condition,p.product
				FROM tools t
      ...
      return $request;
		...
		public function list_materials(int $id){
			$this->intId = $id;
			$sql = "SELECT t.id,t.facilityid,t.productid,t.quantity,t.price,t.total,t.product_condition,p.product,pc.category
			FROM tools t
			JOIN products p ON t.productid = p.id
		...
		public function create_material(int $facility,int $product,int $quantity,string $price,string $total,string $conditions){
			$this->intId = $facility;
			$this->intProduct = $product;
			...
			$answer = "";
			$sql = "SELECT *FROM tools WHERE facilityid = $this->intId AND productid = $this->intProduct";
			$request = $this->select_all($sql);
			if(empty($request)){
				$query = "INSERT INTO tools(facilityid,productid,quantity,price,total,product_condition) VALUES (?,?,?,?,?,?)";
				$data = array($this->intId,$this->intProduct,$this->intQuantity,$this->strPricePro,$this->strTotalPro,$this->strConditions);
				$insert = $this->insert($query,$data);
				if($insert){
					$answer = 'success';
				}else{
      ...
      return $answer;
		...
		public function remove_image(int $facility, string $imagen){
			...
			$this->strImagen = $imagen;
			$answer = "";
			$query  = "DELETE FROM gallery_images WHERE typeid = $this->intFacility AND image = ?";
			$data_check = array($this->strImagen);
			$delete = $this->delete($query,$data_check);
			if($delete){
				$answer = 'success';
			}else{
				$answer = 'error';

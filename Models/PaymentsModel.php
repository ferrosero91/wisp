<?php
	class PaymentsModel extends Mysql{
		...
		public function list_records(string $start,string $end,string $type,string $user,string $state){
				if (isset($start) && isset($end) && isset($type) && isset($user) && isset($state) && !empty($start) && !empty($end) && !empty($type) && !empty($user) && !empty($state)){
						...
						WHERE DATE(p.payment_date) >= ? AND DATE(p.payment_date) <= ? AND p.paytypeid = ? AND p.userid = ? AND p.state = ? ORDER BY p.id DESC";
						$data = array($start,$end,$type,$user,$state);
				}elseif(isset($start) && isset($end) && empty($type) && empty($user) && empty($state)){
						$sql = "SELECT p.id,p.clientid,p.billid,p.internal_code,fp.payment_type,p.payment_date,p.comment,p.amount_paid,p.amount_total,p.remaining_credit,p.state,b.total AS bill_total,CONCAT_WS(' ', c.names, c.surnames) AS client,u.names AS user,vs.serie,v.voucher,b.correlative
						FROM payments p
						JOIN forms_payment fp ON p.paytypeid = fp.id
						JOIN users u ON p.userid = u.id
						...
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE DATE(p.payment_date) >= ? AND DATE(p.payment_date) <= ? ORDER BY p.id DESC";
						$data = array($start,$end);
				}elseif(isset($start) && isset($end) && isset($type) && empty($user) && empty($state)){
						$sql = "SELECT p.id,p.clientid,p.billid,p.internal_code,fp.payment_type,p.payment_date,p.comment,p.amount_paid,p.amount_total,p.remaining_credit,p.state,b.total AS bill_total,CONCAT_WS(' ', c.names, c.surnames) AS client,u.names AS user,vs.serie,v.voucher,b.correlative
						FROM payments p
						JOIN forms_payment fp ON p.paytypeid = fp.id
						JOIN users u ON p.userid = u.id
						...
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE DATE(p.payment_date) >= ? AND DATE(p.payment_date) <= ? AND p.paytypeid = ? ORDER BY p.id DESC";
						$data = array($start,$end,$type);
				}elseif(isset($start) && isset($end) && empty($type) && isset($user) && empty($state)){
						$sql = "SELECT p.id,p.clientid,p.billid,p.internal_code,fp.payment_type,p.payment_date,p.comment,p.amount_paid,p.amount_total,p.remaining_credit,p.state,b.total AS bill_total,CONCAT_WS(' ', c.names, c.surnames) AS client,u.names AS user,vs.serie,v.voucher,b.correlative
						FROM payments p
						JOIN forms_payment fp ON p.paytypeid = fp.id
						JOIN users u ON p.userid = u.id
						...
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE DATE(p.payment_date) >= ? AND DATE(p.payment_date) <= ? AND p.userid = ? ORDER BY p.id DESC";
						$data = array($start,$end,$user);
				}elseif (isset($start) && isset($end) && empty($type) && empty($user) && isset($state)){
						$sql = "SELECT p.id,p.clientid,p.billid,p.internal_code,fp.payment_type,p.payment_date,p.comment,p.amount_paid,p.amount_total,p.remaining_credit,p.state,b.total AS bill_total,CONCAT_WS(' ', c.names, c.surnames) AS client,u.names AS user,vs.serie,v.voucher,b.correlative
						FROM payments p
						JOIN forms_payment fp ON p.paytypeid = fp.id
						JOIN users u ON p.userid = u.id
						...
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE DATE(p.payment_date) >= ? AND DATE(p.payment_date) <= ? AND p.state = ? ORDER BY p.id DESC";
						$data = array($start,$end,$state);
				}elseif(isset($start) && isset($end) && isset($type) && isset($user) && empty($state)){
					$sql = "SELECT p.id,p.clientid,p.billid,p.internal_code,fp.payment_type,p.payment_date,p.comment,p.amount_paid,p.amount_total,p.remaining_credit,p.state,b.total AS bill_total,CONCAT_WS(' ', c.names, c.surnames) AS client,u.names AS user,vs.serie,v.voucher,b.correlative
					FROM payments p
					JOIN forms_payment fp ON p.paytypeid = fp.id
					JOIN users u ON p.userid = u.id
					...
					JOIN vouchers v ON b.voucherid = v.id
					JOIN voucher_series vs ON b.serieid = vs.id
					WHERE DATE(p.payment_date) >= ? AND DATE(p.payment_date) <= ? AND p.paytypeid = ? AND p.userid = ? ORDER BY p.id DESC";
					$data = array($start,$end,$type,$user);
				}elseif(isset($start) && isset($end) && isset($type) && empty($user) && isset($state)){
					$sql = "SELECT p.id,p.clientid,p.billid,p.internal_code,fp.payment_type,p.payment_date,p.comment,p.amount_paid,p.amount_total,p.remaining_credit,p.state,b.total AS bill_total,CONCAT_WS(' ', c.names, c.surnames) AS client,u.names AS user,vs.serie,v.voucher,b.correlative
					FROM payments p
					JOIN forms_payment fp ON p.paytypeid = fp.id
					JOIN users u ON p.userid = u.id
					...
					JOIN vouchers v ON b.voucherid = v.id
					JOIN voucher_series vs ON b.serieid = vs.id
					WHERE DATE(p.payment_date) >= ? AND DATE(p.payment_date) <= ? AND p.paytypeid = ? AND p.state = ? ORDER BY p.id DESC";
					$data = array($start,$end,$type,$state);
			    }elseif(isset($start) && isset($end) && empty($type) && isset($user) && isset($state)){
						$sql = "SELECT p.id,p.clientid,p.billid,p.internal_code,fp.payment_type,p.payment_date,p.comment,p.amount_paid,p.amount_total,p.remaining_credit,p.state,b.total AS bill_total,CONCAT_WS(' ', c.names, c.surnames) AS client,u.names AS user,vs.serie,v.voucher,b.correlative
						FROM payments p
						JOIN forms_payment fp ON p.paytypeid = fp.id
						JOIN users u ON p.userid = u.id
						...
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE DATE(p.payment_date) >= ? AND DATE(p.payment_date) <= ? AND p.userid = ? AND p.state = ? ORDER BY p.id DESC";
						$data = array($start,$end,$user,$state);
				}else{
						$sql = "SELECT p.id,p.clientid,p.billid,p.userid,p.clientid,p.internal_code,fp.payment_type,p.payment_date,p.comment,p.amount_paid,p.amount_total,p.remaining_credit,p.state,b.total AS bill_total,CONCAT_WS(' ', c.names, c.surnames) AS client,u.names AS user,vs.serie,v.voucher,b.correlative
						FROM payments p
						JOIN forms_payment fp ON p.paytypeid = fp.id
						JOIN users u ON p.userid = u.id
						JOIN clients c ON p.clientid = c.id
				...
				$answer = $this->select_all($sql,$data);
				return $answer;
    	}
		...
		public function search_clients(string $search){
			$sql = "SELECT * FROM clients WHERE names LIKE '%$search%' OR surnames LIKE '%$search%' OR document LIKE '%$search%'";
		...
		public function pending_invoices(int $client){
			...
			if($total >= 1){
				...
				$request = array('pending' => $total,'amount' => $amount,'client' => $request_client);
			}
			return $request;
		}
      ...
      JOIN vouchers v ON b.voucherid = v.id
      JOIN voucher_series vs ON b.serieid = vs.id
      WHERE b.state NOT IN(0,1,4) AND b.clientid = $this->intClient ORDER BY b.id ".$ordering;
      $asnwer = $this->select_all($sql);
      return $asnwer;
		}
		...
		public function modify_amounts(string $id,string $subscriber,string $remaining,int $state){
			$this->intBill = $id;
			$this->strSubscriber = $subscriber;
			$this->strRemaining = $remaining;
			$this->intState = $state;

<?php
    Class CurrencysModel extends Mysql{
        ...
        public function create(string $iso,string $language,string $currency,string $money,string $money_plural,string $symbol,string $datetime,int $state){
            ...
            $this->intState = $state;
            $answer = "";
            $sql = "SELECT *FROM currency WHERE currency_name = ? AND money = ? AND money_plural = ?";
            $data_check = array($this->strCurrency,$this->strMoney,$this->strMoneyPlural);
        ...
        public function modify(int $id,string $iso,string $language,string $currency,string $money,string $money_plural,string $symbol,int $state){
            ...
            $this->intState = $state;
            $answer = "";
            $sql = "SELECT *FROM currency WHERE currency_name = ? AND money = ? AND money_plural = ? AND id != $this->intId";
            $data_check = array($this->strCurrency,$this->strMoney,$this->strMoneyPlural);
            $request = $this->select_all($sql,$data_check);
            if(empty($request)){

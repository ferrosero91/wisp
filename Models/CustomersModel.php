<?php
  Class CustomersModel extends Mysql{
    ...
    private $intService;
    /* VARIABLES CLIENTES */
    private $intClient,$strNames,$strSurnames,$intType,$strDocument,$strMobile,$strMobileOp,$strEmail,$strAddess,$strReference,$strLatitud,$strLongitud;
    /* VARIABLS USUARIOS */
    private $intUser;
    /* VARIABLS INSTALACION */
    private $intTechnical,$strInsDate,$strInsHour,$strInsPrice,$strDetail,$strService,$strImagen,$strUtil,$strCodeServ,$strTypeServ,$strRise,$strPriceServ,$intFacility;
    ...
    public function import(string $user,string $client,string $code,string $payday,string $invoice,string $daygrace,string $discount,string $price,string $month,string $datetime,string $state){
      $this->intUser = $user;
      $this->intClient = $client;
      $this->strCode = $code;
      $this->intPayDay = $payday;
    ...
    public function modify(int $id,int $payday,int $invoice,int $daygrace,int $discount,string $price,string $month,int $state){
      $this->intId = $id;
      $this->intPayDay = $payday;
      $this->intInvoice = $invoice;
      $this->intDayGrace = $daygrace;
      ...
      $this->intState = $state;
      $answer = "";
      $query = "UPDATE contracts SET payday=?,create_invoice=?,days_grace=?,discount=?,discount_price=?,months_discount=?,remaining_discount=?,state=? WHERE id = $this->intId";
      $data = array($this->intPayDay,$this->intInvoice,$this->intDayGrace,$this->intDiscount,$this->strPrice,$this->intMonth,$this->intMonth,$this->intState);
    ...
    public function import_client(string $names,string $surnames,int $type,string $document,string $mobile,string $mobileOp,string $email,string $address,string $reference,string $latitud,string $longitud){
      $this->strNames = $names;
      ...
      $answer = "";
      $query = "INSERT INTO clients(names,surnames,documentid,document,mobile,mobile_optional,email,address,reference,latitud,longitud) VALUES(?,?,?,?,?,?,?,?,?,?,?)";
      $data = array($this->strNames,$this->strSurnames,$this->intType,$this->strDocument,$this->strMobile,$this->strMobileOp,$this->strEmail,$this->strAddess,$this->strReference,$this->strLatitud,$this->strLongitud);
      $insert = $this->insert($query,$data);
      if($insert){
        $answer = 'success';
      }else{
    ...
    public function create_client(string $names,string $surnames,int $type_document,string $document,string $mobile,string $mobileOp,string $email,string $address,string $reference){
      $this->strNames = $names;
      $this->strSurnames = $surnames;
      ...
      $this->strReference = $reference;
      $answer = "";
      $query = "INSERT INTO clients(names,surnames,documentid,document,mobile,mobile_optional,email,address,reference) VALUES(?,?,?,?,?,?,?,?,?)";
      $data = array($this->strNames,$this->strSurnames,$this->intType,$this->strDocument,$this->strMobile,$this->strMobileOp,$this->strEmail,$this->strAddess,$this->strReference);
      $insert = $this->insert($query,$data);
      if($insert){
        $answer = 'success';
      }else{
    ...
    public function modify_client(int $id,string $names,string $surnames,int $type_document,string $document,string $mobile,string $mobileOp,string $email,string $address,string $reference,string $latitud,string $longitud){
      $this->intClient = $id;
      $this->strNames = $names;
      $this->strSurnames = $surnames;
      $this->intType = $type_document;
      ...
      $this->strLongitud = $longitud;
      $answer = "";
      $sql = "SELECT *FROM clients WHERE names = ? AND surnames = ? AND id != $this->intClient";
      $data_check = array($this->strNames,$this->strSurnames);
      $request = $this->select_all($sql,$data_check);
      if(empty($request)){
        $query = "UPDATE clients SET names=?,surnames=?,documentid=?,document=?,mobile=?,mobile_optional=?,email=?,address=?,reference=?,latitud=?,longitud=? WHERE id = $this->intClient";
        $data = array($this->strNames,$this->strSurnames,$this->intType,$this->strDocument,$this->strMobile,$this->strMobileOp,$this->strEmail,$this->strAddess,$this->strReference,$this->strLatitud,$this->strLongitud);
        $update = $this->update($query,$data);
        if($update){
          $answer = 'success';
    ...
    public function existing_client(string $names,string $surnames){
      $this->strNames = $names;
      $this->strSurnames = $surnames;
      $query = "SELECT * FROM contracts WHERE clientid IN(SELECT id FROM clients WHERE names = ? AND surnames = ? AND state = 1)";
      $data_check = array($this->strNames,$this->strSurnames);
      $answer = $this->select_all($query,$data_check);
      return $answer;
    }
    public function list_documents(){
      $sql = "SELECT *FROM document_type WHERE id != 1";
      $request = $this->select_all($sql);
      return $request;
    }
    ...
    public function search_document(string $document){
      $this->strDocument = $document;
      $sql = "SELECT * FROM clients WHERE document = ?";
      $data_check = array($this->strDocument);
      $request = $this->select($sql,$data_check);
      return $request;
    }
    ...
    public function bill_voucher(int $id){
      $request = array();
      $sql_bill = "SELECT b.id,b.clientid,b.voucherid,b.serieid,c.names,c.surnames,dt.document AS type_doc,c.document,c.mobile,c.address,c.email,v.voucher,vs.serie,b.internal_code,b.observation,b.date_issue,b.billed_month,b.correlative,b.expiration_date,b.subtotal,b.discount,b.total,b.amount_paid,b.remaining_amount,b.type,b.sales_method,b.state FROM bills b JOIN clients c ON b.clientid = c.id JOIN document_type dt ON c.documentid = dt.id JOIN users u ON b.userid = u.id JOIN vouchers v ON b.voucherid = v.id JOIN voucher_series vs ON b.serieid = vs.id WHERE b.id = $id";
    ...
    public function create_ticket(int $user,int $client,int $technical,int $incidents,string $description,int $priority,string $attention,string $datetime){
      ...
      $this->strDatetime = $datetime;
      $answer = "";
      $sql = "SELECT *FROM tickets WHERE attention_date = ? AND clientid  = $this->intClient AND state != 6";
      $data_check = array($this->strAttention);
      $request = $this->select_all($sql,$data_check);
      if(empty($request)){
    ...
    public function modify_ticket(int $id,int $client,int $technical,int $incidents,string $description,int $priority,string $attention){
      ...
      $this->strAttention = $attention;
      $answer = "";
      $sql = "SELECT *FROM tickets WHERE attention_date = ? AND clientid = $this->intClient AND id != $this->intId";
      $data_check = array($this->strAttention);
      $request = $this->select_all($sql,$data_check);
      if(empty($request)){
    ...
    public function existing_service(string $service){
      ...
      $data_check = array($this->strService);
      $answer = $this->select($query,$data_check);
      return $answer;
    }
    ...
    public function import_detail_facility(string $facility,string $technical,string $insDate){
      $this->intFacility = $facility;
      $this->intTechnical = $technical;
      $this->strInsDate = $insDate;
      $answer = "";
      $query = "INSERT INTO detail_facility(facilityid,technicalid,opening_date,closing_date,state)VALUES(?,?,?,?,?)";
      $data = array($this->intFacility,$this->intTechnical,$this->strInsDate,$this->strInsDate,1);
      $insert = $this->insert($query,$data);
      if($insert){
        $answer = 'success';
      }else{
    ...
    public function remove_image(int $client, string $imagen){
      $this->intClient = $client;
      $this->strImagen = $imagen;
      $query  = "DELETE FROM gallery_images WHERE clientid = $this->intClient AND image = ?";

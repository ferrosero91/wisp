<?php
    Class VouchersModel extends Mysql{
        ...
        public function create(string $voucher,string $datetime,int $state){
            ...
            $this->intState = $state;
            $answer = "";
            $sql = "SELECT *FROM vouchers WHERE voucher = ?";
            $data_check = array($this->strVoucher);
        ...
        public function modify(int $id,string $voucher,int $state){
            ...
            $this->intState = $state;
            $answer = "";
            $sql = "SELECT *FROM vouchers WHERE voucher = ? AND id != $this->intId";
            $data_check = array($this->strVoucher);
            $request = $this->select_all($sql,$data_check);
            if(empty($request)){
        ...
        public function add_serie(string $date,string $serie,string $fromc,string $until,int $idvouchers,string $available){
            ...
            $answer = "";
            $sql = "SELECT *FROM voucher_series WHERE serie = ?";
            $data_check = array($this->strSerie);
            $request = $this->select_all($sql,$data_check);
            if(empty($request)){
        ...
        public function edit_serie(int $id,string $date,string $serie,string $fromc,string $until,int $idvouchers,string $available){
            ...
            $this->strAvailable = $available;
            $answer = "";
            $sql = "SELECT * FROM voucher_series WHERE serie = ? AND id != $this->intIdSerie";
            $data_check = array($this->strSerie);
            $request = $this->select_all($sql,$data_check);
            if(empty($request)){

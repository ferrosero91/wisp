<?php
	class DashboardModel extends Mysql{
		...
		public function number_customers(){
			$sql = "SELECT COUNT(*) AS quantity FROM contracts WHERE state != 0";
		...
		public function total_transactions_day(string $day){
			$sql = "SELECT COALESCE(SUM(amount_paid),0) AS total FROM payments WHERE DATE(payment_date) = ? AND state = 1";
			$data = array($day);
		...
		public function total_transactions_month(){
			$sql = "SELECT COALESCE(SUM(amount_paid),0) AS total FROM payments WHERE MONTH(payment_date) = MONTH(NOW()) AND state = 1";
			$answer = $this->select($sql);
			$total = $answer['total'];
			return $total;
		}
		...
		public function transactions_month(int $month,int $year){
			...
			for ($i=1; $i < $days ; $i++){
				...
				$query = "SELECT DAY(payment_date) AS day,COUNT(id) AS quantity,COALESCE(SUM(amount_paid),0) AS total FROM payments WHERE DATE(payment_date) = ? AND state = 1";
				$data = array($formatted_date);
				$payments = $this->select($query,$data);
				$payments['day'] = $day;
		...
		public function payments_type(int $month,int $year){
			...
			WHERE MONTH(p.payment_date) = ? AND YEAR(p.payment_date) = ? AND p.state = 1 GROUP BY fp.payment_type ORDER BY COUNT(p.paytypeid) DESC LIMIT 6";
			$data = array($month,$year);
			$answer = $this->select_all($query,$data);

<?php
	class BillsModel extends Mysql{
		...
		private $intMonth,$intYear,$intMethod,$strMonth,$strSubscriber,$strTotalPaid,$strRemaining;
		...
		public function list_records(string $start,string $end,string $state){
				if(isset($start) && isset($end) && isset($state) && !empty($start) && !empty($end) && !empty($state)){
						$sql = "SELECT b.id,b.clientid,CONCAT_WS(' ', c.names, c.surnames) AS client,b.internal_code,b.correlative,b.date_issue,b.expiration_date,b.billed_month,b.discount,b.subtotal,b.total,b.type,b.sales_method,b.remaining_amount,b.amount_paid,b.state,vs.serie,v.voucher,b.observation
						FROM bills b
						JOIN clients c ON b.clientid = c.id
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE b.state != 0 AND DATE(b.date_issue) >= ? AND DATE(b.date_issue) <= ? AND b.state = ? ORDER BY b.id DESC";
						...
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE b.state != 0 AND DATE(b.date_issue) >= ? AND DATE(b.date_issue) <= ? ORDER BY b.id DESC";
		...
		public function select_record(int $id){
			$this->intId = $id;
			$sql = "SELECT b.id,b.clientid,CONCAT_WS(' ', c.names, c.surnames) AS client,b.internal_code,b.correlative,b.date_issue,b.expiration_date,b.subtotal,b.discount,b.total,b.amount_paid,b.remaining_amount,b.type,b.sales_method,b.billed_month,b.state,vs.serie,v.voucher,b.observation,b.voucherid,b.serieid
            FROM bills b
			JOIN clients c ON b.clientid = c.id
            JOIN vouchers v ON b.voucherid = v.id
            JOIN voucher_series vs ON b.serieid = vs.id
		...
		public function existing_client(string $names,string $surnames){
				$sql = "SELECT * FROM clients WHERE names = ? AND surnames = ?";
				$answer = $this->select($sql,array($names,$surnames));
				return $answer;
		}
		...
		public function create_payment(int $bill,int $user,int $client,string $code,int $type,string $datetime,string $comment,string $subscriber,string $total_paid,string $remaining,int $state){
							...
							$data = array($this->intId,$this->intUser,$this->intClient,$this->strCodePayment,$this->strTypePayment,$this->strDatetime,$this->strComment,$this->strSubscriber,$this->strTotalPaid,$this->strRemaining,$this->intState);
							$insert = $this->insert($query,$data);
							if($insert){
									$answer = 'success';
							}else{

...
let table_name="list",table_name_bills="list-bills";
document.addEventListener('DOMContentLoaded', function(){
    table_configuration('#'+table_name,'Lista de tickets');
    table = $('#'+table_name).DataTable({
          "ajax":{
            "url": " "+base_url+"/tickets/list_records?state="+$('#listStates').val()+"&user="+$('#listUsers').val()+"&affair="+$('#listfilAffairs').val(),
            "dataSrc": ""
          },
          "deferRender": true,
          "idDataTables": "1",
          "columns": [
              ...
              {"data": "client","render": function(data,type,full,meta){
                return '<a href="'+base_url+'/customers/view_client/'+full.encrypt_contract+'">'+data+'</a>';
              }},
              ...
              {"data": "coordinates","render": function(data,type,full,meta){
                  ...
                  if(data == ""){
                      ...
                      coordinates = '<a href="'+base_url+'/tickets/client_location/'+full.encrypt_client+'"><i class="fa fa-map-marker-alt mr-1 text-danger"></i>'+data+'</a>';
                  }
                  return coordinates;
              }},
        ...
        if (processing) {
            loaderin('.panel-tickets');
        }else{
            loaderout('.panel-tickets');
        }
    });
    if(document.querySelector("#transactions")){
        ...
        transactions.onsubmit = function(e){
            ...
            if($('#transactions').parsley().isValid()){
                ...
                if(client == "") {
                    alert_msg("info","No seleccionaste ningún cliente, seleccione uno.");
                     return false;
                }
                if(affairs == "") {
                    ...
                    if(request.readyState == 4 && request.status == 200){
                        ...
                        if(objData.status == "success"){
                          ...
                          if(objData.modal){
                            ...
                            document.querySelector('#text_country').innerHTML = "+"+objData.country_code;
                            document.querySelector('#country_code').value = objData.country_code;
...
window.addEventListener('load', function(){
    ...
    $('#listPriority,#listStates,#listPriorities').select2({
  ...
  $('[data-toggle="tooltip"]').tooltip('hide');
   ...
   request.onreadystatechange = function(){
       if(request.readyState == 4 && request.status == 200){
           var objData = JSON.parse(request.responseText);
           if(objData.status == "success"){
              document.querySelector('#text-view').innerHTML = "Ticket Nº "+objData.data.ticket.code;
              document.querySelector('#view-client').innerHTML = objData.data.ticket.client;
              document.querySelector('#view-celdoc').innerHTML = "<b>"+objData.data.ticket.type_doc+":</b> "+objData.data.ticket.document+" <b>CEL:</b> "+objData.data.ticket.mobile;
              document.querySelector('#view-address').innerHTML = objData.data.ticket.address;
              document.querySelector("#view-image-client").src = generateAvatar(objData.data.ticket.client);
              document.querySelector('#view-created').innerHTML = "Creado "+moment(objData.data.ticket.registration_date).format('DD/MM/YYYY H:mm');
              document.querySelector('#view-visit').innerHTML = "Programado "+moment(objData.data.ticket.attention_date).format('DD/MM/YYYY H:mm');
              if(objData.data.ticket.priority == 1){
                document.querySelector('#view-priority').innerHTML = '<span style="color:#00acac;border: 1px solid #00acac;padding: 2px 5px;border-radius: 5px;text-transform: uppercase;font-size: 9.5px;font-weight: 700;">BAJA</span>';
                document.querySelector('#view-star').innerHTML ='<i class="fas fa-star"></i>';
              }else if(objData.data.ticket.priority == 2){
                document.querySelector('#view-priority').innerHTML = '<span style="color:#4da1ff;border: 1px solid #4da1ff;padding: 2px 5px;border-radius: 5px;text-transform: uppercase;font-size: 9.5px;font-weight: 700;">MEDIA</span>';
                ...
                document.querySelector('#view-priority').innerHTML = '<span style="color:#F59C1A;border: 1px solid #F59C1A;padding: 2px 5px;border-radius: 5px;text-transform: uppercase;font-size: 9.5px;font-weight: 700;">ALTA</span>';
                ...
                document.querySelector('#view-priority').innerHTML = '<span style="color:#ff5959;border: 1px solid #ff5959;padding: 2px 5px;border-radius: 5px;text-transform: uppercase;font-size: 9.5px;font-weight: 700;">URGENTE</span>';
                ...
                objData.data.images.forEach((image) => {
                  images++;
                    ...
                    <div class="image-inner">
                      <a href="${image.url_image}" data-lightbox="gallery-group-1">
                        <div class="img" style="background-image: url(${image.url_image})"></div>
                      </a>
                    </div>
                    <div class="image-info p-2">
                        <div class="pull-right"><strong>${moment(image.registration_date).format('DD/MM/YYYY')}</strong> ${moment(image.registration_date).format('h:mm')} <strong>${moment(image.registration_date).format('a')}</strong></div>
                        <div class="rating"><small class="mr-1">por</small><strong>${image.names}</strong></div>
                     </div>
                  </div>`;
                });
              }
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status == "success"){
                ...
                document.querySelector('#text_country').innerHTML = "+"+objData.data.country_code;
                document.querySelector('#country_code').value = objData.data.country_code;
                document.querySelector('#idpdfticket').value = objData.data.encrypt;
                document.querySelector('#bill_number_client').value = objData.data.mobile;
...
function modal_tools(country,number,client){
  ...
  document.querySelector('#text-title-tools').innerHTML = client+" - "+number;
                      ...
                      if(request.readyState == 4 && request.status == 200){
                          ...
                          if(objData.status == 'success'){
                            if(objData.data.state == 1){
                              if(user_profile == 1){
                                $(location).attr('href', base_url+"/tickets/finalize/"+idticket);
                              }else{
                                $('[data-toggle="tooltip"]').tooltip('hide');
                              ...
                              refresh_table();
                            }else if(objData.data.state == 2 || objData.data.state == 3 || objData.data.state == 4 || objData.data.state == 5){
                              $(location).attr('href', base_url+"/tickets/finalize/"+idticket);
                            }else{
                              $('[data-toggle="tooltip"]').tooltip('hide');
...
$('#btn-search').on('click',function(){
    table.ajax.url(base_url+"/tickets/list_records?state="+$('#listStates').val()+"&user="+$('#listUsers').val()+"&affair="+$('#listfilAffairs').val(),).load();
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status == "success"){
                ...
                document.querySelector("#listClients").value = objData.data.encrypt_client;
                $('#listClients').select2();
...
function filter_incidents(){
    if(document.querySelector('#listAffairs')){
        var ajaxUrl = base_url+'/incidents/filter_incidents';
        var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
        request.open("GET",ajaxUrl,true);
        request.send();
        request.onreadystatechange = function(){
            if(request.readyState == 4 && request.status == 200){
                document.querySelector('#listfilAffairs').innerHTML = request.responseText;
                $('#listfilAffairs').select2();
            }
        }
    }
}
function list_users(){
    if(document.querySelector('#listUsers')){
        var ajaxUrl = base_url+'/tickets/filter_technical';
        var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
        request.open("GET",ajaxUrl,true);
        request.send();
        request.onreadystatechange = function(){

          ...
          "columns": [
              ...
              {"data": "client","render": function(data,type,full,meta){
                ...
                if(full.profile_user == 1){
                  client = '<a href="'+base_url+'/customers/view_client/'+full.encrypt_contract+'">'+data+'</a>';
                }else{
                  client = data;
                }
                return client;
              ...
              {"data":"state","render": function(data,type,full,meta){
                  ...
                  if(data == 4){
                    state = '<span class="label label-secondary">NO INSTALADO</span>';
                  }
                  if(data == 5){
...
function filter_states(){
    table.ajax.url(base_url+"/installations/list_records/"+$('#filter_states').val()).load();
      ...
      if(request.readyState == 4 && request.status == 200){
          ...
          if(objData.status == "success"){
            if(objData.data.state == 5){
              alert_msg("info","No se puedo arregar materiales a esta instalación por que ya fue cancelada.");
              refresh_table();
            }else{
              $(location).attr('href', base_url+"/installations/tools/"+idfacility);
            }
          }else{
          alert_msg("error",objData.msg);
        }
                      ...
                      if(request.readyState == 4 && request.status == 200){
                          ...
                          if(objData.status == 'success'){
                            if(objData.data.state == 1){
                              ...
                              refresh_table();
                            }else if(objData.data.state == 2 || objData.data.state == 3 || objData.data.state == 4){
                              $(location).attr('href', base_url+"/installations/attend/"+idfacility);
                            }else{
                              $('[data-toggle="tooltip"]').tooltip('hide');
  ...
  $('[data-toggle="tooltip"]').tooltip('hide');
   ...
   request.onreadystatechange = function(){
       if(request.readyState == 4 && request.status == 200){
           var objData = JSON.parse(request.responseText);
           if(objData.status == "success"){
              document.querySelector('#text-view').innerHTML = "Instalación Nº "+objData.data.facility.code;
              document.querySelector('#view-client').innerHTML = objData.data.facility.client;
              document.querySelector('#view-celdoc').innerHTML = "<b>"+objData.data.facility.type_doc+":</b> "+objData.data.facility.document+" <b>CEL:</b> "+objData.data.facility.mobile;
              document.querySelector('#view-address').innerHTML = objData.data.facility.address;
              document.querySelector("#view-image-client").src = generateAvatar(objData.data.facility.client);
              document.querySelector('#view-created').innerHTML = "Creado "+moment(objData.data.facility.registration_date).format('DD/MM/YYYY H:mm');
              document.querySelector('#view-visit').innerHTML = "Programado "+moment(objData.data.facility.attention_date).format('DD/MM/YYYY H:mm');
              ...
              document.querySelector('#view-description').innerHTML = objData.data.facility.detail;
              var data_service = '';
              ...
              if(objData.data.facility.user_image == "user_default.png"){
                ...
                user_image = base_url+"/Assets/uploads/users/"+objData.data.facility.user_image;
              }
              document.querySelector('#view-user-post').src = user_image;
              document.querySelector('#view-user').innerHTML = "<b>"+objData.data.facility.user+"</b>";
              var comments = 0;
              var data_detail = '';
  ...
  request.onreadystatechange = function(){
    if(request.readyState == 4 && request.status == 200){
      ...
      if(objData.status == "success"){
        ...
        document.querySelector("#insDate").value = moment(objData.data.attention_date).format('DD/MM/YYYY H:mm');
...
$('#btn-whatsapp').on('click',function(){
    var country = document.querySelector('#tools_country').value;
    var number = document.querySelector('#tools_number').value;
    var client = document.querySelector('#tools_client').value;
    let message = "";

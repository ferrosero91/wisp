...
document.addEventListener('DOMContentLoaded', function(){
    ...
    table = $('#'+table_name).DataTable({
          "ajax":{
            "url": " "+base_url+"/payments/list_records?start="+$('#start').val()+"&end="+$('#end').val()+"&type="+$('#listPayments').val()+"&user="+$('#listUsers').val()+"&state="+$('#listStates').val(),
            "dataSrc": ""
          },
          ...
          "drawCallback": function(settings){
              var received_amount = 0,received_total = 0;
              var canceled_amount = 0,canceled_total = 0;
              ...
              $('.received_total').html(currency_symbol+roundNumber(received_total,2));
...
function updateAmountSelected(){
  $('.'+table_name+'_amount_selected').html(Object.keys(payment_data_selected).length)
}
...
function export_excel(){
  ...
  setTimeout(function(){
    ...
    window.open(base_url+'/payments/export_excel?start='+$('#start').val()+"&end="+$('#end').val()+"&type="+$('#listPayments').val()+"&user="+$('#listUsers').val()+"&state="+$('#listStates').val(), '_blank');
  }, 1000);
}
function export_pdf(){
  ...
  setTimeout(function(){
    ...
    window.open(base_url+'/payments/export_pdf?start='+$('#start').val()+"&end="+$('#end').val()+"&type="+$('#listPayments').val()+"&user="+$('#listUsers').val()+"&state="+$('#listStates').val(), '_blank');
  }, 1000);
}
...
function filter_runway(){
    if(document.querySelector('#listPayments')){
        ...
        request.onreadystatechange = function(){
            if(request.readyState == 4 && request.status == 200){
                document.querySelector('#listPayments').innerHTML = request.responseText;
...
$('#btn-search').on('click',function(){
    table.ajax.url(base_url+"/payments/list_records?start="+$('#start').val()+"&end="+$('#end').val()+"&type="+$('#listPayments').val()+"&user="+$('#listUsers').val()+"&state="+$('#listStates').val()).load();
...
function view_bill(idbill){
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            var objData = JSON.parse(request.responseText);
            if(objData.status == "success"){
                document.querySelector('#text-view').innerHTML = "Detalle de factura ("+objData.data.bill.voucher+" Nº "+objData.data.bill.serie+")";
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status == "success"){
                ...
                document.querySelector("#date_time").value = moment(objData.data.payment_date).format('DD/MM/YYYY H:mm');
...
function cancel_massive(){
  ...
  if(send_data.length >= 1){
    var alsup = $.confirm({
        ...
        title: 'CONFIRMACIÓN',
        content: 'Cancelar (<b class="text-warning">'+num_pay+'</b>) pagos seleccionados',
        buttons: {
            cancel: {
                text: 'Aceptar',
                btnClass: 'btn-info',

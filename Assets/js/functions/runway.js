...
document.addEventListener('DOMContentLoaded', function(){
    table_configuration('#'+table_name,'Lista de pasarelas');
    table = $('#'+table_name).DataTable({
          "ajax":{
            "url": " "+base_url+"/runway/list_records",
            "dataSrc": ""
          },
          "deferRender": true,
          "idDataTables": "1",
    ...
    if(document.querySelector("#transactions")){
        ...
        transactions.onsubmit = function(e){
            ...
            if($('#transactions').parsley().isValid()){
                ...
                var ajaxUrl = base_url+'/runway/action';
...
function update(idrunway){
    $('[data-toggle="tooltip"]').tooltip('hide');
    $('#transactions').parsley().reset();
    document.querySelector('#text-title').innerHTML = "Actualizar forma de pago";
    document.querySelector('#text-button').innerHTML ="Guardar Cambios";
    var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    var ajaxUrl = base_url+'/runway/select_record/'+idrunway;
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            var objData = JSON.parse(request.responseText);
            if(objData.status == "success"){
                document.querySelector("#idrunway").value = objData.data.encrypt_id;
                document.querySelector("#runway").value = objData.data.payment_type;
        ...
        buttons: {
            remove: {
                ...
                action: function () {
                    ...
                    var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
                    var ajaxUrl = base_url+'/runway/remove';
                    var strData = "idrunway="+idrunway;
                    request.open("POST",ajaxUrl,true);
...
function modal(){
    document.querySelector('#text-title').innerHTML = "Nueva forma de pago";
    document.querySelector('#text-button').innerHTML ="Guardar Registro";
    document.querySelector('#idrunway').value ="";

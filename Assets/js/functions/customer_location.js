...
function initMap(){
    ...
    latLng =	new google.maps.LatLng(parseFloat(lat_client),parseFloat(lng_client));
    ...
    var infowindow = new google.maps.InfoWindow( { content: "<h5 class='text-center f-w-600 mb-0'>"+name_client+"</h5>"+address} );
    ...
    navigator.geolocation.getCurrentPosition(function(position,location_error,options_coord)  {
      ...
      directionsService.route({
          ...
          if (status == 'OK') {
              ...
              infowindow.setContent("<h5 class='text-center f-w-600 mb-0'>Mi ubicación actual</h5>");
              infowindow.setPosition(response.routes[0].legs[0].start_location);
              infowindow.open(map);

              var infowindow2 = new google.maps.InfoWindow();
              infowindow2.setContent("<h5 class='text-center f-w-600 mb-0'>"+name_client+"</h5>"+address);
              infowindow2.setPosition(response.routes[0].legs[0].steps[step].end_location);
              infowindow2.open(map);
          } else {
              alert("ERROR GOOGLE: <br>"+status);
          }
      });
    }, function() {
        var start = new google.maps.LatLng(-8.381723950980284,-74.54314678745268);
        var end = new google.maps.LatLng(lat_client, lng_client);
        ...
        directionsService.route({
            ...
            if (status == 'OK') {
                ...
                var infowindow2 = new google.maps.InfoWindow();
                infowindow2.setContent("<h5 class='text-center f-w-600 mb-0'>"+name_client+"</h5><b>Distancia:</b> "+response.routes[0].legs[0].distance.text+"<br>"+address);
                infowindow2.setPosition(response.routes[0].legs[0].steps[step].end_location);
                infowindow2.open(map);
            } else {
                alert("ERROR GOOGLE: <br>"+status);

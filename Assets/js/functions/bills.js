...
document.addEventListener('DOMContentLoaded', function(){
  ...
  table = $('#'+table_name).DataTable({
        "ajax":{
          "url": " "+base_url+"/bills/list_records?start="+$('#start').val()+"&end="+$('#end').val()+"&state="+$('#listStates').val(),
          "dataSrc": ""
        },
        "deferRender": true,
        "idDataTables": "1",
        "columns": [
            ...
            {"data": "client","render": function(data,type,full,meta){
                ...
                if(full.encrypt_contract == ""){
                    ...
                    var client = '<a href="'+base_url+'/customers/view_client/'+full.encrypt_contract+'">'+data+'</a>';
                }
                return client;
            }},
            ...
            {"data":"payment_date"},
            {"data":"waytopay"},
            {"data":"sales_method","render": function(data,type,full,meta){
                var sales_method = '';
                if(data == 1){
        ...
        "drawCallback": function(settings){
            var paid_amount = 0,paid_subtotal = 0,paid_total = 0;
            ...
            $('.paid_total').html(currency_symbol+roundNumber(paid_total,2));
            $('.unpaid_amount').html(unpaid_amount);
            $('.unpaid_subtotal').html(currency_symbol+roundNumber(unpaid_subtotal,2));
            $('.unpaid_total').html(currency_symbol+roundNumber(unpaid_total,2));
            $('.expired_amount').html(expired_amount);
            $('.expired_subtotal').html(currency_symbol+roundNumber(expired_subtotal,2));
            $('.expired_total').html(currency_symbol+roundNumber(expired_total,2));
                ...
                if(objData.status == "success"){
                    ...
                    if(objData.modal){
                      $("#modal-free").on("hidden.bs.modal", function (){
                        ...
                        document.querySelector('#country_code').value = objData.country;
                        document.querySelector('#bill_number_client').value = objData.mobile;
                        var url = base_url+"/invoice/document/"+objData.idbill+"/ticket";
                        if(objData.remaining_amount == 0){
                            if(objData.type == 1){
                            var msg = `Hola, se registro ${objData.symbol+objData.amount_paid} al ${objData.voucher.toLowerCase()} número ${objData.serie} del cliente ${objData.client}. Puede revisarlo en el siguiente enlace: ${url}. Muchas gracias por su pago, Atte. ${objData.business_name}`;
                            }else{
...
window.addEventListener('load', function(){
    ...
    $('#start,#end,#freeissue,#freeexpiration,#servissue,#servexpiration').datetimepicker({
      locale: 'es',
      format: 'L'
    });
    $('#start').val(moment().startOf('month').format('DD/MM/YYYY'));
    $('#end').val(moment().endOf('month').format('DD/MM/YYYY'));
    $('#freeissue,#freeexpiration,#servissue,#servexpiration').val(moment().format('DD/MM/YYYY'));
...
function type_document(value){
    ...
    if(item){
        var input = document.querySelector("#doc_search");
...
$('#btn-search').on('click',function(){
    table.ajax.url(base_url+"/bills/list_records?start="+$('#start').val()+"&end="+$('#end').val()+"&state="+$('#listStates').val()).load();
...
function bill_free(){
    ...
    $('#listMethod').select2({minimumResultsForSearch: -1});
    document.querySelector("#cont-iss-free").setAttribute("class","col-md-3 form-group");
    document.querySelector("#cont-exp-free").setAttribute("class","col-md-3 form-group");
...
function send_whatsapp(idbill){
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            var objData = JSON.parse(request.responseText);
            if(objData.status == "success"){
                var cell = objData.data.bill.mobile;
                if(cell == "" || cell == null){
                    alert_msg("warning","El cliente no tiene número de teléfono registrado.");
                    return;
                }
                var url = base_url+"/invoice/document/"+objData.data.bill.encrypt_bill;
                var msg;
                if(objData.data.bill.type == 2){
                    var dateAr = objData.data.bill.billed_month.split('-');
                    var month = dateAr[1];
                    msg = `Hola, esta pendiente el pago de ${objData.data.business.symbol+objData.data.bill.remaining_amount} del ${objData.data.bill.voucher.toLowerCase()} número ${objData.data.bill.serie} de ${month_letters(month).toUpperCase()} del cliente ${objData.data.bill.client}. Puede revisarlo en el siguiente enlace: ${url}. Muchas gracias por su pago, Atte. ${objData.data.business.business_name}`;
                }else{
                    msg = `Hola, esta pendiente el pago de ${objData.data.business.symbol+objData.data.bill.remaining_amount} del ${objData.data.bill.voucher.toLowerCase()} número ${objData.data.bill.serie} del cliente ${objData.data.bill.client}. Puede revisarlo en el siguiente enlace: ${url}. Muchas gracias por su pago, Atte. ${objData.data.business.business_name}`;
                }
                var country = objData.data.business.country_code;
                var wurl = "https://api.whatsapp.com/send/?phone="+country+cell+"&text="+encodeURIComponent(msg);
                window.open(wurl);
            }else{
                alert_msg("error",objData.msg);
            }
  ...
  request.onreadystatechange = function(){
      if(request.readyState == 4 && request.status == 200){
          ...
          if(objData.status == "success"){
              ...
              document.querySelector("#cont-client-temp-serv").style.display = "block";
              document.querySelector('#idclient_temp').value = objData.data.bill.names+" "+objData.data.bill.surnames;
              document.querySelector("#idclient_temp").readOnly = true;
              document.querySelector("#cont-client-serv").style.display = "none";
              document.querySelector('#client_serv').value = objData.data.bill.encrypt_client;
              $('#client_serv').select2();
...
function modal_debtOpening(){
    $('[data-toggle="tooltip"]').tooltip('hide');
    document.querySelector('#btn-massive').innerHTML = '<i class="fas fa-save mr-2"></i>Emitir Facturas';
    debt_opening(moment().format('MM-YYYY'));
    table_detail_opening(moment().format('MM-YYYY'));
    document.querySelector('#text-title-massive').innerHTML = "Facturar servicios de "+month_letters(moment().format('MM'))+", "+moment().format('YYYY');
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status == "success"){
                ...
                var month = dateAr[0];
                var year = dateAr[1];
                document.querySelector('#text-title-massive').innerHTML = "Facturar servicios de "+month_letters(month)+", "+year;
                document.querySelector("#total_clients").innerHTML = "("+objData.data.total_clients+")";
                document.querySelector("#total_collection").innerHTML = "("+objData.data.issued_invoices+")";
            }else{
                var dateAr = params.split('-');
                var month = dateAr[0];
                var year = dateAr[1];
...
function table_detail_opening(params){
    ...
    var month = dateAr[0];
    var year = dateAr[1];
    table_configuration('#list-client','Facturar servicios de '+month_letters(month)+", "+year);
    table_client = $('#list-client').DataTable({
          "ajax":{
...
function modal_import(){
    $('[data-toggle="tooltip"]').tooltip('hide');
    document.querySelector('#text-title-import').innerHTML = "Importar Facturas - Servicios";

    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status == "success"){
                ...
                document.querySelector("#listProfiles").value = objData.data.encrypt_profile;
...
function list_profiles(){
    if(document.querySelector('#listProfiles')){
        ...
        request.onreadystatechange = function(){
            if(request.readyState == 4 && request.status == 200){
                document.querySelector('#listProfiles').innerHTML = request.responseText;
                $('#listProfiles').select2({width: '100%'});
            }
        }
    }
...
function search_document(){
    ...
    if(doc != ''){
        if(type == 2){
            ...
            document.querySelector("#names").value = "";
            document.querySelector("#surnames").value = "";
            alert_msg("info","La busqueda solo es para la cédula.");
        }
    }else{
        if(type == 2){
            document.querySelector("#names").value = "";

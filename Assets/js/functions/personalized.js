...
document.addEventListener('DOMContentLoaded', function(){
    table_configuration('#'+table_name,'Lista de planes personalizados');
    table = $('#'+table_name).DataTable({
          "ajax":{
            "url": " "+base_url+"/personalized/list_records",
            "dataSrc": ""
          },
          "deferRender": true,
          "idDataTables": "1",
        ...
        if (processing) {
            loaderin('.panel-personalized');
        }else{
            loaderout('.panel-personalized');
        }
    });
    if(document.querySelector("#transactions")){
        ...
        transactions.onsubmit = function(e){
            ...
            if($('#transactions').parsley().isValid()){
                ...
                var ajaxUrl = base_url+'/personalized/action';
...
function update(idservices){
    ...
    var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    var ajaxUrl = base_url+'/personalized/select_record/'+idservices;
        ...
        buttons: {
            remove: {
                ...
                action: function () {
                    ...
                    var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
                    var ajaxUrl = base_url+'/personalized/remove';

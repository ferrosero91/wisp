...
document.addEventListener('DOMContentLoaded', function(){
    ...
    table = $('#'+table_name).DataTable({
          "ajax":{
            "url": " "+base_url+"/tickets/list_resolved?closing="+$('#closing_date').val(),
            "dataSrc": ""
          },
          "deferRender": true,
          "idDataTables": "1",
          ...
          initComplete: function(oSettings, json){
            ...
            $('#'+table_name+'_wrapper div.container-options').append($('#'+table_name+'-btns-filter').contents());
          }
...
window.addEventListener('load', function(){
    ...
    $('#closing_date').val(moment().format('DD/MM/YYYY'));
    $('#transactions').parsley();
    $('#listPriority').select2({
      minimumResultsForSearch: -1
    });
},false);
$('#closing_date').on('dp.change', function(e){
  table.ajax.url(base_url+"/tickets/list_resolved?closing="+$('#closing_date').val()).load();
...
$('#btn-search').on('click',function(){
    table.ajax.url(base_url+"/tickets/list_records?start="+$('#start').val()+"&end="+$('#end').val()+"&state="+$('#listStates').val()+"&priority="+$('#listPriorities').val()).load();
    //table.columns.adjust().responsive.recalc();
});

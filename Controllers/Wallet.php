<?php
    class Wallet extends Controllers{
        ...
        public function select_record(string $idwallet){
            if($_SESSION['permits_module']['v']){
                $idwallet = decrypt($idwallet);
                $idwallet = intval($idwallet);
                if($idwallet > 0){
                    $data = $this->model->select_record($idwallet);
                    if(empty($data)){
        ...
        public function action(){
            if($_POST){
                if(empty($_POST['wallet'])){
                  ...
                  $id = decrypt($_POST['idwallet']);
                  $id = intval($id);
                    $wallet = strtoupper(strClean($_POST['wallet']));
                    ...
                    if($id == 0){
                        ...
                        if($_SESSION['permits_module']['r']){
                            $request = $this->model->create($wallet,$description,$datetime,$state);
                        }
                    }else{
                        $option = 2;
                        if($_SESSION['permits_module']['a']){
                            $request = $this->model->modify($id,$wallet,$description,$state);
                        }
                    }
                    if($request == "success"){
                        if($option == 1){
        ...
        public function remove(){
            if($_POST){
                if($_SESSION['permits_module']['e']){
                    $idwallet = decrypt($_POST['idwallet']);
                    $idwallet =  intval($idwallet);
                    $request = $this->model->remove($idwallet);
                    if($request == 'success'){
                      $response = array('status' => 'success', 'msg' => 'El registro se ha eliminado.');
                    }else if($request == 'exists'){
                        $response = array('status' => 'exists', 'msg' => 'La cartera esta en uso, imposible eliminar');
                    }else{

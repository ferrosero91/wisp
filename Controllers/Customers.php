<?php
  ...
  class Customers extends Controllers{
      ...
      if(!empty($params)){
        ...
        for($i=0; $i < count($data); $i++){
          ...
          if(!empty($data[$i]['mobile_optional'])){
            $mobiles .= '<br>';
            $mobiles .= '<a href="javascript:;" onclick="modal_tools(\''.encrypt($data[$i]['clientid']).'\',\''.$data[$i]['mobile_optional'].'\')"><i class="fa fa-mobile mr-1"></i>'.$data[$i]['mobile_optional'].'</a>';
          }
    ...
    public function register_contract(){
      if($_POST){
        if(empty($_POST['document']) || empty($_POST['names']) || empty($_POST['surnames']) || empty($_POST['mobile']) || empty($_POST['address']) || empty($_POST['insDate'])){
          ...
          if($_POST['listPlan'] == 1){
            ...
            $invoice = intval($_POST['listInvoice']);
    ...
    public function modify_contract(){
      if($_POST){
        if(empty($_POST['idcontract'])){
          ...
          if($_SESSION['permits_module']['a']){
            $request = $this->model->modify($id,$payday,$invoice,$daygrace,$discount,$pricedisc,$monthdisc,$state);
            if($request == "success"){
              $date =  date("Y-m-d");
              if($state == 1){
                $this->model->modify_date($id,"0000-00-00","0000-00-00");
                $this->model->state_service($id,1);
              }
              if($state == 2){
                $this->model->modify_date($id,"0000-00-00","0000-00-00");
                $this->model->state_service($id,1);
              ...
              if($state == 3){
                $this->model->modify_date($id,$date,"0000-00-00");
                $this->model->state_service($id,2);
              }
              if($state == 4){
                $this->model->modify_date($id,"0000-00-00",$date);
                $this->model->state_service($id,3);
              }
              if($state == 5){
                $this->model->modify_date($id,"0000-00-00","0000-00-00");
                $this->model->state_service($id,1);
          ...
          if($request == "success"){
            $response = array('status' => 'success', 'msg' => 'Se ha actulizado exitosamente.');
          }else{
    ...
    public function modify_client(){
      if($_POST){
        if(empty($_POST['idclient']) || empty($_POST['document']) || empty($_POST['names']) || empty($_POST['surnames']) || empty($_POST['mobile']) || empty($_POST['address'])){
          $response = array("status" => 'error', "msg" => 'Campos señalados son obligatorios.');
        }else{
          ...
          if($_SESSION['permits_module']['a']){
            $request = $this->model->modify_client($idclient,$names,$surnames,$type_document,$document,$mobile,$mobileOp,$email,$address,$reference,$latitud,$longitud);
          }
          if($request == "success"){
    ...
    public function cancel(){
      if($_SESSION['permits_module']['e']){
        if($_POST){
          ...
          $request = $this->model->cancel($idcontract,$date_finish);
          if($request == 'success'){
    ...
    public function layoff(){
      if($_SESSION['permits_module']['e']){
        if($_POST){
          ...
          $request = $this->model->layoff($idcontract,$suspension_date);
          if($request == 'success'){
            $response = array('status' => 'success', 'msg' => 'Se suspendio el servicio al cliente.');
          }else{
            $response = array('status' => 'error', 'msg' => 'Error no se pudo realizar esete proceso.');
          }
          echo json_encode($response,JSON_UNESCAPED_UNICODE);
    ...
    public function activate(){
      if($_SESSION['permits_module']['e']){
        if($_POST){
          ...
          $request = $this->model->activate($idcontract,$date_finish);
          if($request == 'success'){
            $response = array('status' => 'success', 'msg' => 'Se activo el servicio al cliente.');
          }else{
            $response = array('status' => 'error', 'msg' => 'Error no se pudo realizar esete proceso.');
          }
          echo json_encode($response,JSON_UNESCAPED_UNICODE);
    ...
    public function import(){
      ...
      for($i=2; $i <= $row_clients; $i++){
        ...
        $existing_client = $this->model->existing_client($names,$surnames);
        if(empty($existing_client)){
          $request_client = $this->model->import_client($names,$surnames,$type_document,$document,$mobile,$mobile_optional,$email,$address,$reference,$latitud,$longitud);
          if($request_client == "success"){
            ...
            $request = $this->model->import($user,$idclient,$code,$payday,$create_invoice,$days_grace,$discount_value,$discount_price,$months_discount,$datetime,$state);
            if($request > 0){
              ...
              if($request_facility == "success"){
                ...
                $this->model->import_detail_facility($idfacility,$user,$date_installation);
              }
            }
    ...
    public function export(){
      ...
      $spreadsheet->getActiveSheet()->getStyle('A1:P1')->applyFromArray($style_header);
      $center_cell = array(
        'alignment' => array(
              ...
              if($type == 1){
                if($request == "success"){
                  ...
                  if($method == 1){
                    ...
                    $request = $this->model->create_payment($idbill,$user,$client,$code_payment,1,$datetime,"","",$total_bill,0,1);
                    if($request == "success"){
        ...
        if(empty($_POST['idclient']) || empty($_POST['idbill']) || empty($_POST['total_payment'])){
          ...
          if($id == 0){
            ...
            if($_SESSION['permits_module']['r']){
              ...
              $remaining_amount = $this->model->remaining_amount($bill);
              if($remaining_amount == 0){
                $consult_bill = $this->model->select_bill($bill);
                $response = array("status" => 'info', "msg" => 'Este '.strtolower($consult_bill['voucher'])." ya esta pagado.");
              }else{
                $subtract_amounts = $remaining_amount - $subscriber;
                if($subtract_amounts == 0){
                  $this->model->modify_amounts($bill,$subscriber,0,1);
                  $amount_paid = $this->model->amount_paid($bill);
                  $request = $this->model->create_payment($bill,$user,$client,$code,$typepay,$datetime,$comment,$subscriber,$amount_paid,0,1);
                  if($request == "success"){
                    $consult_bill = $this->model->select_bill($bill);
                    $consult_client = $this->model->select_client($client);
                    $response = array('status' => 'success',
                      'msg' => strtolower($consult_bill['voucher']).' Nº '.str_pad($consult_bill['correlative'],7,"0", STR_PAD_LEFT).' ha sido aceptada.',
                      'modal' => true,
                      'idbill' => encrypt($bill),
                      'voucher' => $consult_bill['voucher'],
                      'serie' => $consult_bill['serie']."-".str_pad($consult_bill['correlative'],7,"0", STR_PAD_LEFT),
                ...
                }else if($subtract_amounts > 0){
                  $this->model->modify_amounts($bill,$subscriber,$subtract_amounts,0);
                  $amount_paid = $this->model->amount_paid($bill);
                  $request = $this->model->create_payment($bill,$user,$client,$code,$typepay,$datetime,$comment,$subscriber,$amount_paid,$subtract_amounts,1);
                  if($request == "success"){
                    $consult_bill = $this->model->select_bill($bill);
                    $consult_client = $this->model->select_client($client);
                    $response = array('status' => 'success',
                      'msg' => 'El pago se agrego correctamente, con un saldo de '.$_SESSION['businessData']['symbol'].$subtract_amounts.' pendiente.',
                      'modal' => true,
                      'idbill' => encrypt($bill),
                      'voucher' => $consult_bill['voucher'],
                      'serie' => $consult_bill['serie']."-".str_pad($consult_bill['correlative'],7,"0", STR_PAD_LEFT),
                  ...
                  $response = array("status" => 'warning', "msg" => 'El monto ingresado es mayor al monto a pagar, actualizar la pagina.');
                }
              }
            }
          }else{
            ...
            if($_SESSION['permits_module']['a']){
              ...
              if($request == "success"){
                $response = array('status' => 'success', 'msg' => 'Los cambios fueron guardados correctamente.','modal' => false);
              }else{
                $response = array("status" => 'error', "msg" => 'No se pudo realizar esta operación, intentelo nuevamente.');
              }
            }

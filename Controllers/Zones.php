<?php
    class Zones extends Controllers{
        ...
        public function select_record(string $idzone){
            if($_SESSION['permits_module']['v']){
                $idzone = decrypt($idzone);
                $idzone = intval($idzone);
                if($idzone > 0){
                    $data = $this->model->select_record($idzone);
                    if(empty($data)){
        ...
        public function action(){
            if($_POST){
                if(empty($_POST['zone'])){
                  ...
                  $id = decrypt($_POST['idzone']);
                  $id = intval($id);
                    $zone = strtoupper(strClean($_POST['zone']));
                    ...
                    if($id == 0){
                        ...
                        if($_SESSION['permits_module']['r']){
                            $request = $this->model->create($zone,$description,$datetime,$state);
                        }
                    }else{
                        $option = 2;
                        if($_SESSION['permits_module']['a']){
                            $request = $this->model->modify($id,$zone,$description,$state);
                        }
                    }
                    if($request == "success"){
                        if($option == 1){
        ...
        public function remove(){
            if($_POST){
                if($_SESSION['permits_module']['e']){
                    $idzone = decrypt($_POST['idzone']);
                    $idzone =  intval($idzone);
                    $request = $this->model->remove($idzone);
                    if($request == 'success'){
                      $response = array('status' => 'success', 'msg' => 'El registro se ha eliminado.');
                    }else if($request == 'exists'){
                        $response = array('status' => 'exists', 'msg' => 'La zona esta en uso, imposible eliminar');
                    }else{

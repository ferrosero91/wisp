...
use Dompdf\Dompdf;
    class Installations extends Controllers{
            ...
            if(empty($_SESSION['permits_module']['v'])){
                ...
                for($i=0; $i < count($data); $i++){
                    ...
                    if($_SESSION['permits_module']['v']){
                      ...
                      if($data[$i]['state'] == 1){
                        $email = '<a href="javascript:;" class="blue" data-toggle="tooltip" data-original-title="Enviar por correo" onclick="send_email(\''.encrypt($data[$i]['id']).'\')"><i class="fa fa-share-square"></i></a>';
        ...
        public function complete_installation(){
          if($_POST){
            if(empty($_POST['idfacility']) || empty($_POST['idclient']) || empty($_POST['latitud']) || empty($_POST['longitud']) || empty($_POST['observation'])){
              $response = array("status" => 'errror', "msg" => 'Campos señalados son obligatorios.');
            }else{
              ...
              if($_SESSION['permits_module']['a']){
                ...
                if($state_facility == 1){
                  ...
                  if($_SESSION['userData']['profileid'] == ADMINISTRATOR){
                    $request = $this->model->complete_installation($facility,$iduser,$opening_date,$closing_date,$observation,$state);
                    if($request == "success"){
                      if($radio_option == 1){
            ...
            if($_POST){
                if(empty($_POST['idfacility']) || empty($_POST['listProduct'])){
                  $response = array("status" => 'errror', "msg" => 'Campos señalados son obligatorios.');
                    ...
                    if($_SESSION['permits_module']['r']){
                        $request = $this->model->create_material($facility,$product,$quantity,$price,$total,$conditions);
                        if($request == "success"){
                            $datetime = date("Y-m-d H:i:s");

                ...
                if(empty($_POST['service'])){
                    ...
                    if($id == 0){
                        ...
                        if($_SESSION['permits_module']['r']){
                            $request = $this->model->create($code,$service,$type,$rise,$rise_select,$descent,$descent_select,$price,$details,$datetime,$state);
                        }
                    }else{
                        $option = 2;
                        if($_SESSION['permits_module']['a']){
                            $request = $this->model->modify($id,$service,$rise,$rise_select,$descent,$descent_select,$price,$details,$state);
                        }
                    }
                    if($request == "success"){
                        if($option == 1){

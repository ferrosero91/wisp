    ...
    public function __construct(){
      parent::__construct();
        ...
        public function bills(){
            ...
            $data['page_functions_js'] = "bills.js";
                ...
                if(empty($_POST['idclient']) || empty($_POST['idbill']) || empty($_POST['total_payment'])){
                    ...
                    if($_SESSION['permits_module']['r']){
                         ...
                         if($remaining_amount == 0){
                            $consult_bill = $this->model->select_record($bill);
        ...
        public function import(){
            ...
            for($i=2; $i <= $row_bills; $i++){
                ...
                if(!empty($existing_client)){
                  ...
                  $request = $this->model->import($user,$idclient,$voucher,$serie,$code,$correlative,$issue,$new_expiration,$current,$subtotal,$discount,$total,$type_bill,$method,$observation,$state,$yearVal,$monthVal);
                  if($request > 0){
                      $idbill = $this->model->returnBill();
                      if($state == 1){
                         $row_payment = $this->model->returnCodePayment();
                        ...
                        $request_payment = $this->model->create_payment($idbill,$user,$idclient,$code_payment,$idmethodpay,"",$comment,$payment_amount,$payment_amount,0,1);
